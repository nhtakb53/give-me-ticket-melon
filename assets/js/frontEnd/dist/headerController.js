...
define('js/app/common/service/headerService',[
    "jquery",
    "underscore",
    "placeholder",
    "ticketDomain",
    "ticketUtil"
], function ($,underscore,placeholder, tktDomain, tktUtil) {
    'use strict';

    var exports = {};

    function init() {
       $("#top_search").placeholder(); //ie9 대응
    ...
    /**
     * 상단 로그인, 회원가입 문구 표시 및 로그인&로그아웃 여부 표시
     */
    function displayLoginArea() {

        var _tgtDom = $("#login_area"), _appString = '';

        _tgtDom.empty();

        if( isMelonLogin() ) {

            _appString = '<a id="btnLogout" href="javascript:;" class="btn_gnb btn_g_logout">로그아웃</a>';
            _appString = $( _appString );

            _appString.on('click', function() {
                setCookie("coop","", -9999, "/", MELON.WEBSVC.POC.melonDomain);
                setCookie("coop_funnel","", -9999, "/", MELON.WEBSVC.POC.melonDomain);
                setCookie("corpCodeNo","", -9999, "/", MELON.WEBSVC.POC.melonDomain);   // 기업할인 삭제
                preReservationCookieDel();  //선예매 인증 쿠키값 삭제
                logout("", createUrl());
            });
            ...
            //	kakao ID Display
            if (getMemberType() != '1') {
                $("#name_area").html('<em>' + getDisplayId() + '</em> 님 반갑습니다!');
            } else {
                $("#name_area").html('<span class="icon kakao">카카오계정</span><em>' + getDisplayId() + '</em> 님 반갑습니다!');
            }

        } else {
            _appString = '<a href="javascript:;" class="btn_gnb btn_g_login" id="global_top_ticketLogin_Button">로그인</a>';
            $("#sub_area").html('<a href="'+commonDomain.GLOBAL_MEMBER_MELON_HTTPS+'/muid/family/ticket/join/web/stipulationagreement_inform.htm?cpId='+ melon.getPocId() +'" class="btn_gnb btn_g_join">회원가입</a>');
            $("#name_area").html('');
        }

        _tgtDom.html(_appString);
     ...
     * @returns {string|[]}
     */
    function getCookieSearch(cname) {
        var name = cname;
        ...
        for(var i = 0; i <ca.length; i++) {
            var c = ca[i];
            while (c.charAt(0) == ' ') { //공백 제거
                c = c.substring(1);
            }

            if (c.indexOf(name) == 0) {
                var tempCookieVal = c;
                var tempCookieKeyLen = tempCookieVal.indexOf("=");
                var newCookieKey = c.substring(0, tempCookieKeyLen);

                cookieArray.push(newCookieKey);
            }
        }
     ...
     * 선예매 쿠키값 삭제 함수
     */
    function preReservationCookieDel(){
        try {

            var cookieKeyArr = getCookieSearch("K_USERCOND_");

            if( _.isArray(cookieKeyArr) ) {
                _.each(cookieKeyArr,function(key,i){
                    setCookie(key,"", -9999, "/", MELON.WEBSVC.POC.melonDomain);   // 선예매 인증 쿠키 삭제
                });
            }
    ...
    function loginStrat() {
        ...
        location.replace(commonDomain.GLOBAL_MEMBER_MELON_HTTPS + "/muid/family/ticket/login/web/login_inform.htm?cpId="+melon.getPocId()+"&returnPage="+createUrl() );
    }
    ...
    function createUrl(){
        var result = "";
        var firstType = commonUtil.urlFindFirst();

        switch (firstType) {
            case "performance":  //상품 상세
                var global_common_requestURL = $("#global_common_requestURL").val();
                var prodIdNum = $("#prodIdNum").val();
                result = commonDomain.GLOBAL_TICKET_HTTPS+global_common_requestURL+"?prodId="+prodIdNum;
                break;
            case "plan":  //상품 상세
                var global_common_requestURL = $("#global_common_requestURL").val();
                result = commonDomain.GLOBAL_TICKET_HTTPS+global_common_requestURL;
                break;
            default:
                result = commonDomain.GLOBAL_TICKET_HTTPS+"/main/readingGate.htm";
                break;
        }
    ...
    function resetCookie(){
        var referrer_domain = document.referrer;

        if(referrer_domain.indexOf("ticket.melon.com") == -1 && referrer_domain.indexOf("member.melon.com") == -1){
            var melonDomain = MELON.WEBSVC.POC.melonDomain;
            setCookie("coop","", -9999, "/", melonDomain);
            setCookie("coop_funnel","", -9999, "/", melonDomain);
            setCookie("cbo","", -9999, "/", melonDomain);
        }
    }
     ...
     * @private
     */
    function _searchValidate( pWord ) {
        if( null === pWord ||  1 > pWord.length ) {
            alert('검색어를 입력해 주세요.');
            return false;
        }
        return true;
     ...
     * @param pWord
     */
    exports.searchInfo = function( pWord ) {
        if( _searchValidate(pWord) ) {
            var pText = encodeURIComponent(pWord);
            location.href = commonDomain.GLOBAL_TICKET_HTTPS +'/search/index.htm?q=' + pText;
        }
    };
...
require([
    ...
    "domReady",
    "ticketDomain",
    "js/app/common/service/headerService"
], function($,domReady,tktDomain, headerService) {
    ...
    function buttonInit(){
         ...
         * 상품 상단 LANGUAGE 버튼
         */
        $("#ticket_global").hover(function(){  //in
            $("#link00").show();
            $("#link01").show();
            $("#link02").show();
            $("#link03").show();
            $("#ticket_global").css('border','solid 1px #b4b4b4');
            $("#link00").attr("style","background: url("+commonDomain.GLOBAL_CDN_TICKET_HTTPS+"/resource/image/web/common/top_ticket_global.png) no-repeat 0 -131px;");
        }, function(){ //out
            $("#link00").show();
            $("#link01").hide();
            $("#link02").hide();
            $("#link03").hide();
            $("#ticket_global").css('border','solid 1px #fff');
            $("#link00").attr("style","width:100%;display:block;background-color:transparent;height:30px");
        });
        ...
        $('#btn_top_search').on('click', function(){
            var _w = $('#top_search').val().trim();
            var _d = $('#placeholderValue').val().trim(); //i9 대비

            if( null === _w ||  1 > _w.length || _w == _d) {
                _w =   $('#searchValue').val().trim();
            }

            headerService.service.searchInfo(_w);
        });
         ...
         * 로그인 버튼
         */
        $(document).off('click',"#global_top_ticketLogin_Button")
            .on('click', "#global_top_ticketLogin_Button",function() {
                headerService.service.loginStrat();
            });
    }
...
define("js/app/common/controller/headerController", function(){});
