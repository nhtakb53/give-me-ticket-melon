  ...
  this._action = function () {
    try {
      if (this._config.waitchk != 0) {
        if (
          parseInt(this._config.waitchk, 10) <
          parseInt(NetFunnel.gLastData.nwait, 10)
        ) {
      ...
      if (
        parseInt(NetFunnel.gLastData.nwait, 10) >
        parseInt(NetFunnel.gTotWait, 10)
      ) {
        NetFunnel.gTotWait = NetFunnel.gLastData.nwait;
      }
      var i =
        this._config.width -
        Math.round(
          (NetFunnel.gLastData.nwait / NetFunnel.gTotWait) * this._config.width
        );
  ...
  try {
    if (!NetFunnel.Util.isVirtualWait(NetFunnel.gLastData)) {
      ...
      if (NetFunnel.gControl.getConfig("use_frame_block") == true) {
        NetFunnel.PopupUtil.hideBlockList(
          NetFunnel.gControl.getConfig("frame_block_list")
        );
      }
    }
  } catch (a) {}
...
NetFunnel.countdown = function () {
  ...
  var left_perc = 0;
  var skinObj = NetFunnel.SkinUtil.get(
    NetFunnel.gSkinId,
    NetFunnel.Util.isSmartPhone()
  );
...
NetFunnel.SkinUtil = {
  ...
  get: function (d, a) {
    try {
      ...
      if (
        typeof NetFunnel.Skin[d] == "object" &&
...
if (
  (NetFunnel.BrowserDetect.browser == "Explorer" &&
    NetFunnel.BrowserDetect.version == "6") ||
  NetFunnel.gLogoData == ""
) {
  NetFunnel.tstr +=
    '<b style="font-size:12px;">' + NetFunnel.gLogoText + "</b></a>";
} else {
  NetFunnel.tstr +=
    '<b style="font-size:12px;">' +
    NetFunnel.gLogoText +
    '</b><img style="height:16px;color:black;font-size:11px;" border=0 src="data:image/gif;base64,' +
    NetFunnel.gLogoData +
    '" ></a>';
}
NetFunnel.tstr +=
...
NetFunnel.PopupSetup = function (d, b, c) {
  ...
  if (d != "alert" && typeof b == "object") {
    ...
    NetFunnel.gLastData.real_time_left = Math.round(
      parseInt(b.data.nwait, 10) / NetFunnel.gLastData.tps
    );
  ...
  if (NetFunnel.gControl.getConfig("use_frame_block") == true) {
    NetFunnel.PopupUtil.showBlockList(
      NetFunnel.gControl.getConfig("frame_block_list")
    );
  }
  if (d != "alert") {
...
NetFunnel.DefaultCallback = {
  ...
  onContinued: function (b, a, c) {
    ...
    if (
      a.rtype == NetFunnel.RTYPE_CHK_ENTER ||
  ...
  onBlock: function (b, a, c) {
    ...
    if (
      typeof c.getConfig("block_url") != "string" ||
      c.getConfig("block_url") == ""
    ) {
      if (
        c.getConfig("block_msg") == "" ||
  ...
  showWaitPopup: function () {
    if (typeof NetFunnel == "object") {
      ...
      if (
        ...
        a +=
          '<td><img style="" border=0 src="data:image/gif;base64,' +
          NetFunnel.gPreWaitData +
          '" ></td>';
      }
      a +=
...
NetFunnel.Popup = function (f, i, d, b, q, h, n) {
  ...
  if (!p) {
    ...
    p.style.width = "100%";
    p.style.height =
      NetFunnel.PopupUtil.getDocumentEntireHeight(this._mDoc) + "px";
    p.style.margin = "0";
    p.style.padding = "0";
...
NetFunnel.Popup.prototype._centerPopWin = function () {
  if (this._mIsShown) {
    ...
    if (typeof this._mTop == "number") {
      ...
      this._mObj.style.top =
        a + (b - NetFunnel.PopupUtil.getObjHeight(this._mObj)) / 2 + "px";
    }
    if (typeof this._mLeft == "number") {
      this._mObj.style.left = this._mLeft + "px";
    } else {
      this._mObj.style.left =
        c + (f - NetFunnel.PopupUtil.getObjWidth(this._mObj)) / 2 + "px";
    }
...
NetFunnel.TsClient = function (b, c) {
  ...
  if (typeof this.mConfig.use_unfocus != "boolean") {
    if (
      typeof this.mConfig.use_unfocus == "string" &&
  ...
  if (typeof this.mConfig.use_mobile_ui != "boolean") {
    if (
      typeof this.mConfig.use_mobile_ui == "string" &&
  ...
  if (typeof this.mConfig.use_frame_block != "boolean") {
    if (
      typeof this.mConfig.use_frame_block == "string" &&
  ...
  if (this.mConfig.use_frame_block == true) {
    if (this.mConfig.frame_block_list.length < 1) {
      this.mConfig.frame_block_list = NetFunnel.Util.getFrameWindowList(
        this.mConfig.popup_target
      );
    }
  } else {
  ...
  this.connTimeout = function d() {
    ...
    if (
      this._mReqType == NetFunnel.RTYPE_CHK_ENTER ||
...
NetFunnel.TsClient.prototype._showResultChkEnter = function (b) {
  ...
  switch (b.getRetCode()) {
    ...
    default:
      ...
      this._mStatus = NetFunnel.PS_ERROR;
      if (
        b.getRetCode() == NetFunnel.kTsErrorInvalidIp &&
  ...
  switch (b.getRetCode()) {
    ...
    case NetFunnel.kContinue:
      ...
      if (a > 0 && this._mStatus != NetFunnel.PS_N_RUNNING) {
        ...
        this.retryTimer = setTimeout(function () {
          c.aliveNoticeCont(
            c.retval.getValue("key"),
            c.retval.getValue("ip"),
            c.retval.getValue("port"),
            c.retval.getValue("first")
          );
        }, a * 1000);
      }
      break;
...
NetFunnel.TsClient.prototype.getTidChkEnter = function (a, c, e, d) {
  ...
  NetFunnel.retryData = { user_id: a, user_tid: c, first: e };
  this.retryTimer = setTimeout(function () {
...
NetFunnel.TsClient.prototype.setComplete = function (h, f, d, e) {
  ...
  if (h == NetFunnel.CONN_TIMEOUT_KEY) {
    var b = new NetFunnel.RetVal(
      NetFunnel.RTYPE_SET_COMPLETE + ":" + NetFunnel.kSuccess + ":utime=1"
    );
...
function NetFunnel_setComplete(f, e) {
  ...
  if (typeof e != "undefined" && e.constructor == Object) {
    ...
    var c = a.getReqType();
    if (
      h != NetFunnel.kSuccess &&
      h != NetFunnel.kTsBypass &&
      !(c == NetFunnel.RTYPE_ALIVE_NOTICE && h == NetFunnel.kContinue)
    ) {
      var i = new NetFunnel.RetVal(
        NetFunnel.RTYPE_SET_COMPLETE + ':200:msg="Success"'
      );
...
function NetFunnel_AliveNotice(f, c) {
  ...
  if (typeof c == "function") {
    NetFunnel_aliveNotice({ success: c, error: c, bypass: c, continued: c });
  } else {
    if (typeof c == "object") {
      var j = c.success;
      var b = c.continued;
